...
{
	@Override
	public void invoke(final AjaxRequestTarget target, final EventSubscription subscription,
			final Object base, final AtmosphereEvent event,
			final AjaxRequestInitializer ajaxRequestInitializer)
	{
		for (final Method curMethod : base.getClass().getMethods())
		{
					...
					&& curMethod.getName().equals(subscription.getMethodName()))
			{
				...
				System.out.println("{{{ errors: false");
			}
		}
	}
}
